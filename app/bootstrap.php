...
{
    return all("SELECT DISTINCT u.* FROM users u JOIN user_roles ur ON ur.user_id=u.id JOIN roles r ON r.id=ur.role_id WHERE u.is_active=1 AND r.slug IN ('admin','gsl','chairperson','qm')");
}
