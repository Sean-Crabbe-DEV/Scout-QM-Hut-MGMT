...
if [[ -z "$SOURCE_DIR" || ! -f "$SOURCE_DIR/public/index.php" ]]; then
  echo "Usage: sudo bash deploy-update.sh /path/to/extracted/scout-hut-mgmt-v1.13"
  exit 1
fi
...
if command -v mysqldump >/dev/null 2>&1; then
  mysqldump --single-transaction --routines --triggers \
    ...
    | gzip > "$BACKUP_DIR/database-before-v1.13-${STAMP}.sql.gz"
fi
cp "$APP_DIR/.env" "$BACKUP_DIR/env-before-v1.13-${STAMP}.backup"
...
if [[ -f "$APP_DIR/composer.json" ]]; then
  if command -v composer >/dev/null 2>&1; then
    if ! composer install --working-dir="$APP_DIR" --no-dev --prefer-dist --optimize-autoloader --no-interaction; then
      echo "Warning: Composer dependencies could not be installed. PDF exports still work because v1.13 uses the built-in PDF renderer; SMTP email needs Composer to be fixed separately."
    fi
  else
...
echo "Scout Hut Management v1.13 deployed."
echo "Backup: $BACKUP_DIR/database-before-v1.13-${STAMP}.sql.gz"
