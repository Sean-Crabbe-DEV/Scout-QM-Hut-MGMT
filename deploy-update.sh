...
if [[ -z "$SOURCE_DIR" || ! -f "$SOURCE_DIR/public/index.php" ]]; then
  echo "Usage: sudo bash deploy-update.sh /path/to/extracted/scout-hut-mgmt-v1.9"
  exit 1
fi
...
if command -v mysqldump >/dev/null 2>&1; then
  mysqldump --single-transaction --routines --triggers \
    ...
    | gzip > "$BACKUP_DIR/database-before-v1.9-${STAMP}.sql.gz"
fi
cp "$APP_DIR/.env" "$BACKUP_DIR/env-before-v1.9-${STAMP}.backup"
...
echo "Scout Hut Management v1.9 deployed."
echo "Backup: $BACKUP_DIR/database-before-v1.9-${STAMP}.sql.gz"
