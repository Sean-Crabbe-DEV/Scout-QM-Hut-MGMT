...
function pdf_asset_data_uri(string $relativePath): ?string
{
...
function equipment_booking_pdf_html(array $booking, array $items): string
{
    $group = setting('group_name', env('APP_NAME', 'Scout Hut Management'));
    $logo = is_file(__DIR__ . '/assets/brand/group-logo-red.svg')
        ? pdf_asset_data_uri('assets/brand/group-logo-red.svg')
        : pdf_asset_data_uri('assets/brand/group-logo-red.png');
    $printedAt = date('d M Y, H:i');
    $holderName = trim((string)($booking['holder_name'] ?? ''));
...
body { margin: 0; color: #1d1d1b; font-family: DejaVu Sans, Arial, sans-serif; font-size: 9pt; line-height: 1.35; }
.header { padding: 0 0 10px; border-bottom: 3px solid #ed3f23; }
.brand-table, .meta-table, .items-table, .signatures { width: 100%; border-collapse: collapse; }
.brand-table td { vertical-align: top; }
.logo-cell { width: 48mm; }
.logo { width: 42mm; height: 13mm; object-fit: contain; object-position: left center; }
.group-name { margin: 0 0 2px; color: #ed3f23; font-size: 7.5pt; font-weight: bold; letter-spacing: .6pt; text-transform: uppercase; }
h1 { margin: 0; font-size: 16pt; line-height: 1.15; }
.subtitle { margin: 3px 0 0; color: #5f5f5f; font-size: 8.5pt; }
.reference { text-align: right; }
.reference-label, .label { display: block; color: #5f5f5f; font-size: 7pt; font-weight: bold; letter-spacing: .45pt; text-transform: uppercase; }
.reference-value { display: block; margin-top: 2px; color: #003982; font-family: DejaVu Sans Mono, monospace; font-size: 10pt; font-weight: bold; }
.meta-table { margin-top: 10px; border: 1px solid #d9d9d9; }
.meta-table td { width: 50%; padding: 7px 8px; border-right: 1px solid #d9d9d9; border-bottom: 1px solid #d9d9d9; vertical-align: top; }
.meta-table tr:last-child td { border-bottom: 0; }
.meta-table td:nth-child(2n) { border-right: 0; }
.meta-value { display: block; margin-top: 2px; font-weight: bold; }
.meta-small { display: block; margin-top: 1px; color: #5f5f5f; font-size: 7.8pt; }
.holder { margin-top: 8px; padding: 7px 8px; border: 1px solid #d9d9d9; background: #f8f8f8; }
.holder strong { margin-left: 5px; }
.items-table { margin-top: 10px; border: 1px solid #9d9894; table-layout: fixed; }
.items-table th { padding: 6px 5px; background: #f3f0ef; border-bottom: 1.5px solid #9d9894; color: #262321; font-size: 7.3pt; text-align: left; text-transform: uppercase; }
.items-table td { min-height: 12mm; padding: 6px 5px; border-bottom: 1px solid #d8d4d1; vertical-align: middle; overflow-wrap: break-word; }
.items-table tr:nth-child(even) td { background: #fcfbfb; }
.items-table tr:last-child td { border-bottom: 0; }
.col-check { width: 9%; text-align: center !important; }
.col-asset { width: 19%; }
.col-item { width: 42%; }
.col-qty { width: 10%; text-align: center !important; }
.col-condition { width: 20%; }
.check-box { display: inline-block; width: 5.2mm; height: 5.2mm; border: 1.4px solid #322f2d; }
.asset-id { color: #003982; font-family: DejaVu Sans Mono, monospace; font-size: 8pt; font-weight: bold; }
.item-name { font-weight: bold; }
.item-category { display: block; margin-top: 2px; color: #5f5f5f; font-size: 7.2pt; }
.qty { font-size: 10pt; font-weight: bold; }
.condition { display: block; min-height: 6mm; border-bottom: 1px dashed #8d8884; font-size: 7.5pt; font-weight: bold; }
.notes { width: 100%; margin-top: 10px; border-collapse: separate; border-spacing: 8px 0; margin-left: -8px; margin-right: -8px; }
.notes td { width: 50%; vertical-align: top; }
.write-area { height: 30mm; margin-top: 4px; border: 1px solid #bcb8b5; }
.signatures { margin-top: 10px; }
.signatures td { width: 33.33%; padding-right: 7px; vertical-align: top; }
.signature-line { height: 12mm; border-bottom: 1px solid #5d5855; }
.footer-note { margin: 9px 0 0; color: #514c49; font-size: 7.2pt; }
</style>
</head>
<body>
  <div class="header">
    <table class="brand-table"><tr>
      <td class="logo-cell"><?php if ($logo): ?><img class="logo" src="<?= e($logo) ?>" alt="<?= e($group) ?>"><?php else: ?><div style="font-size:20pt;color:#ed3f23">⚜</div><?php endif; ?></td>
      <td><p class="group-name"><?= e($group) ?></p><h1>Equipment Booking Summary</h1><p class="subtitle">Kit issue and return checklist</p></td>
      <td class="reference"><span class="reference-label">Booking reference</span><span class="reference-value"><?= e($booking['reference']) ?></span></td>
    </tr></table>
  </div>

  <table class="meta-table">
    <tr><td><span class="label">Event</span><span class="meta-value"><?= e($booking['title']) ?></span></td><td><span class="label">Status</span><span class="meta-value"><?= e($booking['status']) ?></span></td></tr>
    <tr><td><span class="label">Booking user</span><span class="meta-value"><?= e($booking['requester_name']) ?></span><span class="meta-small"><?= e($booking['requester_email']) ?></span></td><td><span class="label">Date printed</span><span class="meta-value"><?= e($printedAt) ?></span></td></tr>
    <tr><td><span class="label">Collection from</span><span class="meta-value"><?= e(date('d M Y, H:i', strtotime($booking['starts_at']))) ?></span></td><td><span class="label">Return by</span><span class="meta-value"><?= e(date('d M Y, H:i', strtotime($booking['ends_at']))) ?></span></td></tr>
  </table>

  <?php if ($holderName !== ''): ?><div class="holder"><span class="label" style="display:inline">Equipment currently issued to</span><strong><?= e($holderName) ?></strong><?php if ($holderEmail !== ''): ?> <span class="meta-small" style="display:inline">(<?= e($holderEmail) ?>)</span><?php endif; ?></div><?php endif; ?>

  <table class="items-table">
    <thead><tr><th class="col-check">Check</th><th class="col-asset">Asset ID</th><th class="col-item">Equipment item</th><th class="col-qty">Qty</th><th class="col-condition">Status / condition out</th></tr></thead>
    <tbody>
    <?php foreach ($items as $item):
        $condition = trim((string)($item['condition_out'] ?? ''));
        if ($condition === '') { $condition = $booking['status'] === 'Checked out' ? 'Not recorded' : 'To confirm at issue'; }
        $quantity = $item['quantity_issued'] ?: $item['quantity_approved'] ?: $item['quantity_requested'];
    ?>
      <tr><td class="col-check"><span class="check-box"></span></td><td class="asset-id"><?= e($item['asset_id']) ?></td><td><span class="item-name"><?= e($item['name']) ?></span><?php if ($item['category']): ?><span class="item-category"><?= e($item['category']) ?></span><?php endif; ?></td><td class="col-qty"><span class="qty"><?= e((string)$quantity) ?></span></td><td><span class="condition"><?= e($condition) ?></span></td></tr>
    <?php endforeach; ?>
    </tbody>
  </table>

  <table class="notes"><tr><td><span class="label">Issue notes</span><div class="write-area"></div></td><td><span class="label">Return notes / damage</span><div class="write-area"></div></td></tr></table>
  <table class="signatures"><tr><td><span class="label">Issued by</span><div class="signature-line"></div></td><td><span class="label">Received by</span><div class="signature-line"></div></td><td><span class="label">Returned to</span><div class="signature-line"></div></td></tr></table>
  <p class="footer-note">Please check each item before it leaves and again on return. Report missing, damaged or unsafe equipment through the Hut Management System.</p>
</body>
</html>
    <?php
...
function ticket_form(array $areas, array $equipment, bool $public = false): void
{
    ...
    ?><form method="post" enctype="multipart/form-data" class="card form-grid ticket-form" data-ticket-form data-hut-categories='<?= e(json_encode($hutCategories)) ?>' data-equipment-categories='<?= e(json_encode($equipmentCategories)) ?>'>
      ...
      <div class="full"><label>Add photo or file</label><input type="file" name="attachments[]" multiple accept="image/jpeg,image/png,image/webp,application/pdf,text/plain"><small>You can choose more than one file. JPG, PNG, WebP, PDF or TXT. Maximum <?= e(env('UPLOAD_MAX_MB', '8')) ?> MB.</small></div>
...
function create_ticket_from_request(bool $public): void
{
    ...
    $id=(int)db()->lastInsertId(); if(!empty($_FILES['attachments']))store_ticket_uploads($_FILES['attachments'],$id); q('INSERT INTO ticket_updates (ticket_id,user_id,body,is_internal,new_status) VALUES (?,?,?,?,?)',[$id,$user['id'] ?? null,'Ticket reported.',0,'New']); $ticket=one('SELECT * FROM tickets WHERE id=?',[$id]); audit('Ticket created','ticket',$id,['reference'=>$reference,'source'=>$ticket['source'],'type'=>$type]); notify_ticket_created($ticket);
...
try {
    ...
    if ($path === '/report-problem') {
        ...
        <?php ticket_form(all('SELECT * FROM hut_areas ORDER BY name'), all("SELECT * FROM equipment WHERE current_status <> 'Disposed of' ORDER BY name"), true); page_end(true); exit;
    }
    ...
    if ($path === '/tickets/new') {
        ...
        page_start('New ticket'); heading('Report an issue', 'Create a ticket for a hut or equipment problem.'); ticket_form(all('SELECT * FROM hut_areas ORDER BY name'),all("SELECT * FROM equipment WHERE current_status <> 'Disposed of' ORDER BY name")); page_end(); exit;
    }
    ...
    if ($path==='/equipment/new') {
        require_admin();
        if($method==='POST'){
            validate_csrf();$asset=trim((string)$_POST['asset_id']);$name=trim((string)$_POST['name']);
            if($asset===''||$name==='')throw new RuntimeException('Asset ID and name are required.');
            $status=(string)($_POST['current_status']??'Available'); if(!in_array($status,equipment_status_options(),true))throw new RuntimeException('Choose a valid equipment status.');
            $owned=max(1,(int)$_POST['quantity_owned']);$available=min($owned,max(0,(int)$_POST['quantity_available'])); if($status==='Disposed of')$available=0;
            $condition=(string)($_POST['current_condition']??'Good'); if($status==='Damaged')$condition='Damaged'; elseif($status==='In repair')$condition='Needs attention'; elseif($status==='Disposed of')$condition='Disposed';
            $photo=store_asset_photo($_FILES['photo']??[]);
            q('INSERT INTO equipment(asset_id,name,category,description,manufacturer,model,serial_number,quantity_owned,quantity_available,storage_location,current_status,current_condition,purchase_date,purchase_price,purchase_place,supplier_contact,estimated_value,warranty_expiry,photo_path,last_inspection_date,next_inspection_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[$asset,$name,trim((string)$_POST['category'])?:'Other',trim((string)$_POST['description'])?:null,trim((string)$_POST['manufacturer'])?:null,trim((string)$_POST['model'])?:null,trim((string)$_POST['serial_number'])?:null,$owned,$available,trim((string)$_POST['storage_location'])?:null,$status,$condition,trim((string)$_POST['purchase_date'])?:null,trim((string)$_POST['purchase_price'])?:null,trim((string)$_POST['purchase_place'])?:null,trim((string)$_POST['supplier_contact'])?:null,trim((string)$_POST['estimated_value'])?:null,trim((string)$_POST['warranty_expiry'])?:null,$photo,trim((string)$_POST['last_inspection_date'])?:null,trim((string)$_POST['next_inspection_date'])?:null]);
            $id=(int)db()->lastInsertId(); if(!empty($_FILES['attachments']))store_equipment_uploads($_FILES['attachments'],$id);
            audit('Equipment item created','equipment',$id,['asset_id'=>$asset]);flash('success','Equipment item added.');redirect('/equipment/'.$id);
        }
        page_start('Add equipment');heading('Add equipment','Create a detailed asset record, including photo and purchasing information.');?>
        <form method="post" enctype="multipart/form-data" class="card form-grid"><?=csrf_field()?>
          <div><label>Asset ID <span>*</span></label><input name="asset_id" required placeholder="For example: CAMP-TP-014"></div><div><label>Item name <span>*</span></label><input name="name" required></div><div><label>Category</label><input name="category" placeholder="For example: Camping equipment"></div><div><label>Storage location</label><input name="storage_location"></div><div><label>Quantity owned</label><input type="number" min="1" name="quantity_owned" value="1"></div><div><label>Quantity available</label><input type="number" min="0" name="quantity_available" value="1"></div>
          <div><label>Status</label><select name="current_status"><?php foreach(equipment_status_options() as $x):?><option><?=$x?></option><?php endforeach;?></select></div><div><label>Current condition</label><select name="current_condition"><?php foreach(['Excellent','Good','Fair','Needs attention','Damaged'] as $x):?><option><?=$x?></option><?php endforeach;?></select></div><div><label>Manufacturer</label><input name="manufacturer"></div><div><label>Model</label><input name="model"></div><div><label>Serial number</label><input name="serial_number"></div><div><label>Main item photo</label><input type="file" name="photo" accept="image/jpeg,image/png,image/webp"></div><div><label>Purchase date</label><input type="date" name="purchase_date"></div><div><label>Purchase price (£)</label><input type="number" step="0.01" min="0" name="purchase_price"></div><div><label>Place purchased from</label><input name="purchase_place"></div><div><label>Supplier contact</label><input name="supplier_contact"></div><div><label>Estimated current value (£)</label><input type="number" step="0.01" min="0" name="estimated_value"></div><div><label>Warranty expiry</label><input type="date" name="warranty_expiry"></div><div><label>Last inspection</label><input type="date" name="last_inspection_date"></div><div><label>Next inspection</label><input type="date" name="next_inspection_date"></div><div class="full"><label>Description</label><textarea name="description" rows="5"></textarea></div><div class="full"><label>Additional files</label><input type="file" name="attachments[]" multiple accept="image/jpeg,image/png,image/webp,application/pdf,text/plain"><small>You can choose more than one file. JPG, PNG, WebP, PDF or TXT.</small></div><button class="button primary">Add equipment</button></form><?php page_end();exit;
    }
    ...
    if (preg_match('#^/equipment-attachment/(\d+)$#',$path,$m)&&$method==='GET') {
        if(!can_manage_equipment()){http_response_code(403);exit('You cannot access this equipment file.');}
        $attachment=one('SELECT * FROM equipment_attachments WHERE id=?',[(int)$m[1]]);if(!$attachment){http_response_code(404);exit('File not found.');}
        $file=UPLOAD_PATH.'/'.$attachment['storage_name'];if(!is_file($file)){http_response_code(404);exit('File not found.');}
        header('Content-Type: '.$attachment['mime_type']);header('Content-Length: '.filesize($file));header('Content-Disposition: inline; filename="'.rawurlencode($attachment['original_name']).'"');readfile($file);exit;
    }
    ...
    if (preg_match('#^/equipment/(\d+)/update$#',$path,$m)) {
        if(!can_manage_equipment()){http_response_code(403);exit('Only Admin, GSL, Chairperson and QM can update equipment.');}
        $item=one('SELECT * FROM equipment WHERE id=?',[(int)$m[1]]);if(!$item){http_response_code(404);exit('Equipment item not found.');}
        if($method==='POST'){
            validate_csrf();$asset=trim((string)$_POST['asset_id']);$name=trim((string)$_POST['name']); if($asset===''||$name==='')throw new RuntimeException('Asset ID and item name are required.');
            $duplicate=one('SELECT id FROM equipment WHERE asset_id=? AND id<>?',[$asset,$item['id']]);if($duplicate)throw new RuntimeException('That Asset ID is already in use.');
            $status=(string)($_POST['current_status']??'Available');if(!in_array($status,equipment_status_options(),true))throw new RuntimeException('Choose a valid equipment status.');
            $owned=max(1,(int)$_POST['quantity_owned']);$available=min($owned,max(0,(int)$_POST['quantity_available'])); if($status==='Disposed of')$available=0;
            $condition=(string)($_POST['current_condition']??'Good');if(!in_array($condition,['Excellent','Good','Fair','Needs attention','Damaged'],true))$condition='Good';if($status==='Damaged')$condition='Damaged';elseif($status==='In repair')$condition='Needs attention';elseif($status==='Disposed of')$condition='Disposed';
            $newPhoto=store_asset_photo($_FILES['photo']??[]);$photo=$newPhoto?:$item['photo_path'];
            q('UPDATE equipment SET asset_id=?,name=?,category=?,description=?,manufacturer=?,model=?,serial_number=?,quantity_owned=?,quantity_available=?,storage_location=?,current_status=?,current_condition=?,purchase_date=?,purchase_price=?,purchase_place=?,supplier_contact=?,estimated_value=?,warranty_expiry=?,photo_path=?,last_inspection_date=?,next_inspection_date=? WHERE id=?',[$asset,$name,trim((string)$_POST['category'])?:'Other',trim((string)$_POST['description'])?:null,trim((string)$_POST['manufacturer'])?:null,trim((string)$_POST['model'])?:null,trim((string)$_POST['serial_number'])?:null,$owned,$available,trim((string)$_POST['storage_location'])?:null,$status,$condition,trim((string)$_POST['purchase_date'])?:null,trim((string)$_POST['purchase_price'])?:null,trim((string)$_POST['purchase_place'])?:null,trim((string)$_POST['supplier_contact'])?:null,trim((string)$_POST['estimated_value'])?:null,trim((string)$_POST['warranty_expiry'])?:null,$photo,trim((string)$_POST['last_inspection_date'])?:null,trim((string)$_POST['next_inspection_date'])?:null,$item['id']]);
            $maintenanceDescription=trim((string)($_POST['maintenance_description']??''));if($maintenanceDescription!==''){$costs=[];foreach(['labour_cost','parts_cost','contractor_cost','delivery_cost','other_cost'] as $key)$costs[$key]=max(0,(float)($_POST[$key]??0));$total=array_sum($costs);q('INSERT INTO maintenance_records(equipment_id,recorded_by_user_id,contractor_name,work_date,description,labour_cost,parts_cost,contractor_cost,delivery_cost,other_cost,total_cost,completed,next_due_date) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)',[$item['id'],current_user()['id'],trim((string)($_POST['contractor_name']??''))?:null,(string)($_POST['work_date']??date('Y-m-d')),$maintenanceDescription,$costs['labour_cost'],$costs['parts_cost'],$costs['contractor_cost'],$costs['delivery_cost'],$costs['other_cost'],$total,!empty($_POST['completed'])?1:0,trim((string)($_POST['next_due_date']??''))?:null]);}
            if(!empty($_FILES['attachments']))store_equipment_uploads($_FILES['attachments'],$item['id']);audit('Equipment updated','equipment',$item['id'],['status'=>$status,'maintenance_added'=>$maintenanceDescription!=='']);flash('success','Equipment item updated.');redirect('/equipment/'.$item['id']);
        }
        page_start('Update equipment');heading('Update equipment',$item['asset_id'].' — '.$item['name']);?>
        <form method="post" enctype="multipart/form-data" class="form-grid"><?=csrf_field()?>
          <section class="card form-grid full"><h2 class="full">Item details</h2><div><label>Asset ID <span>*</span></label><input name="asset_id" value="<?=e($item['asset_id'])?>" required></div><div><label>Item name <span>*</span></label><input name="name" value="<?=e($item['name'])?>" required></div><div><label>Category</label><input name="category" value="<?=e($item['category'])?>"></div><div><label>Storage location</label><input name="storage_location" value="<?=e($item['storage_location'])?>"></div><div><label>Quantity owned</label><input type="number" min="1" name="quantity_owned" value="<?=e((string)$item['quantity_owned'])?>"></div><div><label>Quantity available</label><input type="number" min="0" name="quantity_available" value="<?=e((string)$item['quantity_available'])?>"></div><div><label>Status</label><select name="current_status"><?php foreach(equipment_status_options() as $x):?><option <?=$x===$item['current_status']?'selected':''?>><?=$x?></option><?php endforeach;?></select><small>Available, Booked, Damaged, In repair or Disposed of.</small></div><div><label>Current condition</label><select name="current_condition"><?php foreach(['Excellent','Good','Fair','Needs attention','Damaged'] as $x):?><option <?=$x===$item['current_condition']?'selected':''?>><?=$x?></option><?php endforeach;?></select></div><div><label>Manufacturer</label><input name="manufacturer" value="<?=e($item['manufacturer'])?>"></div><div><label>Model</label><input name="model" value="<?=e($item['model'])?>"></div><div><label>Serial number</label><input name="serial_number" value="<?=e($item['serial_number'])?>"></div><div><label>Replace main photo</label><input type="file" name="photo" accept="image/jpeg,image/png,image/webp"></div><div><label>Purchase date</label><input type="date" name="purchase_date" value="<?=e($item['purchase_date'])?>"></div><div><label>Purchase price (£)</label><input type="number" step="0.01" min="0" name="purchase_price" value="<?=e((string)$item['purchase_price'])?>"></div><div><label>Place purchased from</label><input name="purchase_place" value="<?=e($item['purchase_place'])?>"></div><div><label>Supplier contact</label><input name="supplier_contact" value="<?=e($item['supplier_contact'])?>"></div><div><label>Estimated current value (£)</label><input type="number" step="0.01" min="0" name="estimated_value" value="<?=e((string)$item['estimated_value'])?>"></div><div><label>Warranty expiry</label><input type="date" name="warranty_expiry" value="<?=e($item['warranty_expiry'])?>"></div><div><label>Last inspection</label><input type="date" name="last_inspection_date" value="<?=e($item['last_inspection_date'])?>"></div><div><label>Next inspection</label><input type="date" name="next_inspection_date" value="<?=e($item['next_inspection_date'])?>"></div><div class="full"><label>Description</label><textarea name="description" rows="5"><?=e($item['description'])?></textarea></div><div class="full"><label>Additional files</label><input type="file" name="attachments[]" multiple accept="image/jpeg,image/png,image/webp,application/pdf,text/plain"><small>You can choose more than one file. JPG, PNG, WebP, PDF or TXT.</small></div></section>
          <section class="card form-grid full"><h2 class="full">Add maintenance or repair record <small>Optional</small></h2><div><label>Date of work</label><input type="date" name="work_date" value="<?=date('Y-m-d')?>"></div><div><label>Contractor / supplier</label><input name="contractor_name"></div><div class="full"><label>Work completed</label><textarea name="maintenance_description" rows="3" placeholder="Leave blank when you are only updating item details."></textarea></div><?php foreach(['labour_cost'=>'Labour','parts_cost'=>'Parts','contractor_cost'=>'Contractor','delivery_cost'=>'Delivery','other_cost'=>'Other'] as $field=>$label):?><div><label><?=$label?> cost (£)</label><input type="number" step="0.01" min="0" name="<?=$field?>" value="0"></div><?php endforeach;?><div><label>Next review due</label><input type="date" name="next_due_date"></div><div class="full checkbox"><input id="equipment-completed" type="checkbox" name="completed" value="1"><label for="equipment-completed">Work completed and safe to return to normal use</label></div></section>
          <div class="full form-actions"><a class="button secondary" href="/equipment/<?=$item['id']?>">Cancel</a><button class="button primary">Save equipment update</button></div>
    ...
    if ($path==='/equipment-bookings/new') {
        if ($method==='POST') {
            ...
            foreach($requestedIds as $equipmentId){
                ...
                if(!$item || $item['current_status'] !== 'Available' || in_array($item['current_condition'],['Unsafe — do not use','Out of service','Lost','Disposed','Damaged'],true)) throw new RuntimeException('One or more selected items are not available to book.');
        ...
        $items=all("SELECT * FROM equipment WHERE current_status = 'Available' AND current_condition NOT IN ('Unsafe — do not use','Out of service','Lost','Disposed','Damaged') ORDER BY category,name");
    ...
    if (preg_match('#^/equipment-bookings/(\d+)/export\.pdf$#', $path, $m) && $method === 'GET') {
        $booking = one('SELECT * FROM equipment_bookings WHERE id=?', [(int)$m[1]]);
        if (!$booking) { http_response_code(404); exit('Equipment booking not found.'); }
        if (!can_approve_equipment() && (int)$booking['requester_user_id'] !== (int)current_user()['id']) { http_response_code(403); exit('You cannot export this booking.'); }
    ...
    if (preg_match('#^/equipment-bookings/(\d+)/print$#', $path, $m) && $method === 'GET') {
        ...
        <div class="form-actions print-actions"><a class="button secondary" href="/equipment-bookings/<?=$booking['id']?>">Back to booking</a><a class="button primary" href="/equipment-bookings/<?=$booking['id']?>/export.pdf">Download PDF</a><button class="button secondary" onclick="window.print()">Print preview</button></div><?php page_end(); exit;
    }
    ...
    if (preg_match('#^/equipment-bookings/(\d+)$#', $path, $m) && $method === 'GET') {
        ...
        <div class="page-actions"><a class="button primary" href="/equipment-bookings/<?=$booking['id']?>/export.pdf">Download PDF</a><a class="button secondary" href="/equipment-bookings/<?=$booking['id']?>/export.csv">Export CSV</a><a class="button secondary" href="/equipment-bookings/<?=$booking['id']?>/print" target="_blank">Print preview</a></div>
    ...
    if ($path==='/settings') {
        require_admin();
        if($method==='POST'){
            validate_csrf();$action=(string)($_POST['action']??'save_system');
            if($action==='create_hut_area'){
                $name=trim((string)($_POST['name']??''));if($name==='')throw new RuntimeException('Enter a hut area name.');
                q('INSERT INTO hut_areas(name,area_type,description,booking_enabled,capacity,last_inspection_date,next_inspection_date) VALUES(?,?,?,?,?,?,?)',[$name,trim((string)($_POST['area_type']??''))?:'Area',trim((string)($_POST['description']??''))?:null,!empty($_POST['booking_enabled'])?1:0,(int)($_POST['capacity']??0)?:null,trim((string)($_POST['last_inspection_date']??''))?:null,trim((string)($_POST['next_inspection_date']??''))?:null]);
                audit('Hut area created from settings','hut_area',(int)db()->lastInsertId());flash('success','Hut area added.');redirect('/settings#hut-area-configuration');
            }
            foreach(['group_name','mail_from_address','mail_from_name','mail_reply_to','smtp_host','smtp_port','smtp_encryption','smtp_username'] as $key){save_setting($key,trim((string)($_POST[$key]??'')));}
            save_setting('whole_site_enabled',!empty($_POST['whole_site_enabled'])?'1':'0');if(trim((string)($_POST['smtp_password']??''))!=='')save_setting('smtp_password',crypt_value(trim((string)$_POST['smtp_password'])));
            audit('System settings updated','settings',null);flash('success','System and email settings saved.');redirect('/settings');
        }
        $areas=all('SELECT * FROM hut_areas ORDER BY name');
        page_start('System settings');heading('System settings','Manage identity, public reporting, hut areas and outgoing email.');?>
        <section class="settings-layout"><form method="post" class="card form-grid"><?=csrf_field()?><input type="hidden" name="action" value="save_system"><h2 class="full">System identity</h2><div class="full"><label>Group name</label><input name="group_name" value="<?=e(setting('group_name',env('APP_NAME','Scout Hut Management')))?>"><small>Shown in the header, footer and automated emails.</small></div><div class="full checkbox"><input id="whole_site_enabled" type="checkbox" name="whole_site_enabled" value="1" <?=setting('whole_site_enabled','1')==='1'?'checked':''?>><label for="whole_site_enabled">Allow Whole Site booking requests</label></div><h2 class="full">Email sender</h2><div><label>From email address</label><input type="email" name="mail_from_address" value="<?=e(setting('mail_from_address'))?>"></div><div><label>From name</label><input name="mail_from_name" value="<?=e(setting('mail_from_name'))?>"></div><div class="full"><label>Reply-to address</label><input type="email" name="mail_reply_to" value="<?=e(setting('mail_reply_to'))?>"></div><h2 class="full">SMTP</h2><div><label>SMTP host</label><input name="smtp_host" value="<?=e(setting('smtp_host'))?>"></div><div><label>SMTP port</label><input type="number" name="smtp_port" value="<?=e(setting('smtp_port','587'))?>"></div><div><label>Encryption</label><select name="smtp_encryption"><?php foreach(['tls'=>'TLS / STARTTLS','ssl'=>'SSL / SMTPS','none'=>'None'] as $value=>$label):?><option value="<?=$value?>" <?=setting('smtp_encryption','tls')===$value?'selected':''?>><?=$label?></option><?php endforeach;?></select></div><div><label>SMTP username</label><input name="smtp_username" value="<?=e(setting('smtp_username'))?>"></div><div class="full"><label>SMTP password</label><input type="password" name="smtp_password" placeholder="Leave blank to keep the existing password"><small>The password is encrypted using APP_KEY and is never shown again.</small></div><button class="button primary">Save settings</button></form>
        <aside class="settings-side"><article class="card"><h2>Public problem-report link</h2><p>Share this on noticeboards, QR codes and booking emails.</p><div class="copy-row"><input id="public-report-link" value="<?=e(app_url('/report-problem'))?>" readonly><button type="button" class="button secondary" data-copy-target="#public-report-link">Copy link</button></div><p><a href="<?=e(app_url('/report-problem'))?>" target="_blank" rel="noopener">Open public report form ↗</a></p></article><article class="card"><h2>Hut area configuration</h2><p>Add, edit and control booking availability for each area of the site below.</p><a class="button secondary" href="#hut-area-configuration">Manage hut areas</a></article><article class="card"><h2>Email status</h2><p>Email sends use your configured SMTP server. Until it is configured, the system records sends as skipped so failed notifications are visible.</p></article><article class="card"><h2>Branding</h2><p>The Group logo is loaded from <code>public/assets/brand/group-logo-red.svg</code>.</p></article></aside></section>
        <section id="hut-area-configuration" class="card settings-area-config"><div class="section-heading"><div><h2>Hut area configuration</h2><p>Set up the rooms, outside spaces and fixed facilities available for hut bookings.</p></div><span class="selection-count"><?=count($areas)?> configured</span></div><div class="two-col"><form method="post" class="form-grid"><?=csrf_field()?><input type="hidden" name="action" value="create_hut_area"><div><label>Name <span>*</span></label><input name="name" required placeholder="For example: Main Hall"></div><div><label>Area type</label><input name="area_type" placeholder="For example: Room or Outside area"></div><div><label>Capacity</label><input type="number" min="0" name="capacity"></div><div><label>Last inspection</label><input type="date" name="last_inspection_date"></div><div><label>Next inspection</label><input type="date" name="next_inspection_date"></div><div class="checkbox"><input type="checkbox" id="settings_booking_enabled" name="booking_enabled" value="1"><label for="settings_booking_enabled">This area can be booked</label></div><div class="full"><label>Description</label><textarea name="description" rows="4"></textarea></div><button class="button primary">Add hut area</button></form><div><h3>Configured areas</h3><div class="compact-list"><?php foreach($areas as $area):?><a href="/settings/hut-areas/<?=$area['id']?>"><strong><?=e($area['name'])?></strong><span><?=e($area['area_type'])?> · <?=$area['booking_enabled']?'Bookable':'Not bookable'?> <b>Manage →</b></span></a><?php endforeach;?><?php if(!$areas):?><p class="muted">No hut areas configured yet.</p><?php endif;?></div></div></div></section><?php page_end();exit;
    }

    if (preg_match('#^/settings/hut-areas/(\d+)$#',$path,$m)) {
        require_admin();$area=one('SELECT * FROM hut_areas WHERE id=?',[(int)$m[1]]);if(!$area){http_response_code(404);exit('Hut area not found.');}
        if($method==='POST'){
            validate_csrf();$name=trim((string)($_POST['name']??''));if($name==='')throw new RuntimeException('Enter a hut area name.');
            q('UPDATE hut_areas SET name=?,area_type=?,description=?,booking_enabled=?,capacity=?,last_inspection_date=?,next_inspection_date=? WHERE id=?',[$name,trim((string)($_POST['area_type']??''))?:'Area',trim((string)($_POST['description']??''))?:null,!empty($_POST['booking_enabled'])?1:0,(int)($_POST['capacity']??0)?:null,trim((string)($_POST['last_inspection_date']??''))?:null,trim((string)($_POST['next_inspection_date']??''))?:null,$area['id']]);
            audit('Hut area updated from settings','hut_area',$area['id']);flash('success','Hut area configuration saved.');redirect('/settings#hut-area-configuration');
        }
        page_start('Manage hut area');heading('Manage hut area',$area['name'],'Back to settings','/settings#hut-area-configuration');?><form method="post" class="card form-grid"><?=csrf_field()?><div><label>Name <span>*</span></label><input name="name" value="<?=e($area['name'])?>" required></div><div><label>Area type</label><input name="area_type" value="<?=e($area['area_type'])?>"></div><div><label>Capacity</label><input type="number" min="0" name="capacity" value="<?=e((string)$area['capacity'])?>"></div><div><label>Last inspection</label><input type="date" name="last_inspection_date" value="<?=e($area['last_inspection_date'])?>"></div><div><label>Next inspection</label><input type="date" name="next_inspection_date" value="<?=e($area['next_inspection_date'])?>"></div><div class="checkbox"><input type="checkbox" id="area_booking_enabled" name="booking_enabled" value="1" <?=$area['booking_enabled']?'checked':''?>><label for="area_booking_enabled">This area can be booked</label></div><div class="full"><label>Description</label><textarea name="description" rows="5"><?=e($area['description'])?></textarea></div><div class="full form-actions"><a class="button secondary" href="/settings#hut-area-configuration">Cancel</a><button class="button primary">Save hut area</button></div></form><?php page_end();exit;
    }
